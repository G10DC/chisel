...
const OPENER_PLEASANTRIES = /^\s*(sure|of course|certainly|right|ok|okay|well|so|now|great|perfect|got it|alright|anyway|certo|certamente|ovviamente|dunque|quindi|allora|ecco|beh|vabbè|insomma|diciamo|cioè|guarda)[,.!]\s+/gimu;
...
export function terseProseAdvisor(text) {
  ...
  return t
    ...
    .replace(/^\s*(let me|i'll|i will|i am going to|fammi|ti mostro|ti faccio vedere|ti spiego|ora procedo|adesso procedo|adesso vediamo)\s+/iu, '')
    .replace(FILLER, '')
    .replace(/[ \t]{2,}/g, ' ')
    .replace(/\n{3,}/g, '\n\n')
    .trim();
